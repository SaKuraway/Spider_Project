...
class BbaeSpiderMysqlPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            for top_10_holdings, top_10_holdings_portfolio in zip(item['top_10_holdings'], item['top_10_holdings_portfolio']):
                self.try_except(cur.execute("insert into bbae_top_10_holdings_portfolio(ISIN,top_10_holdings,top_10_holdings_portfolio) VALUES(%s,%s,%s)",(item['ISIN'], top_10_holdings, top_10_holdings_portfolio)))

            for top_5_regions, top_5_regions_percent in zip(item['top_5_regions'], item['top_5_regions_percent']):
                self.try_except(cur.execute("insert into bbae_top_5_regions(ISIN,top_5_regions,top_5_regions_percent) VALUES(%s,%s,%s)",(item['ISIN'], top_5_regions, top_5_regions_percent)))
            ...
            for top_5_sectors, top_5_sectors_percent in zip(item['top_5_sectors'], item['top_5_sectors_percent']):
                self.try_except(cur.execute("insert into bbae_top_5_sectors(ISIN,top_5_sectors,top_5_sectors_percent) VALUES(%s,%s,%s)",(item['ISIN'], top_5_sectors, top_5_sectors_percent)))
            ...
            for top_5_holdings, top_5_holdings_sector, top_5_holdings_percent in zip(item['top_5_holdings'],item['top_5_holdings_sector'],item['top_5_holdings_percent']):
                self.try_except(cur.execute("insert into bbae_top_5_holdings(ISIN,top_5_holdings,top_5_holdings_sector,top_5_holdings_percent) VALUES(%s,%s,%s,%s)",
                    (item['ISIN'], top_5_holdings, top_5_holdings_sector, top_5_holdings_percent)))
            if cur.execute("select * from bbae_valuations_and_growth_rates where ISIN=" + "'" + item['ISIN'] + "'" + ";"):
                self.try_except(cur.execute("delete from bbae_valuations_and_growth_rates where ISIN=" + "'" + item['ISIN'] + "'" + ";"))
            ...
            if cur.execute("select * from bbae_world_regions where ISIN=" + "'" + item['ISIN'] + "'" + ";"):
                self.try_except(cur.execute("delete from bbae_world_regions where ISIN=" + "'" + item['ISIN'] + "'" + ";"))
            for world_regions, world_regions_equity, world_regions_relative_to_category in zip(item['world_regions'],
                                                                                               item['world_regions_equity'],
                                                                                               item[
        ...
        #          top_10_holdings_country, top_10_holdings_percent_of_assets)))
            ...
            for fund_manager, manager_start_date, biography in zip(item['fund_managers'], item['manager_start_dates'],item['biographies']):
                if not cur.execute("select * from bbae_fund_managers where ISIN=" + "'" + item['ISIN'] + "'" + " and fund_manager=" + "'" + fund_manager + "'" + " and manager_start_date=" + "'" + manager_start_date + "'" + ";"):
                    self.try_except(cur.execute("insert into bbae_fund_managers(ISIN, fund_advisor, fund_manager, manager_start_date, biography) VALUES(%s,%s,%s,%s,%s)",(item['ISIN'], item['fund_advisor'], fund_manager, manager_start_date, biography.replace('\n', ''))))
                #
    ...
    def close_spider(self, spider):
        tableNameList = ['bbae_funds','bbae_top_10_holdings_portfolio','bbae_top_5_regions','bbae_top_5_sectors','bbae_top_5_holdings','bbae_valuations_and_growth_rates','bbae_valuations_and_growth_rates','bbae_market_capitalisation_equity','bbae_world_regions','bbae_fund_managers']
        print('正在排序数据表中的id列....')
        cur = self.mysql_client.cursor()

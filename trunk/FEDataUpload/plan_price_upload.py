...
class FEDataUpload(object):
    ...
    def browser_login(self, login_url, group_id, groupname, username, password, username_id, password_id, submit_id):
        """
        调用并操作浏览器对象
        :param url: 网址
        :param username: 用户名
    ...
    def start_work(self):
        driver = self.browser_login(login_url=self.login_url,group_id=self.group_id,groupname=self.Groupname,username_id=self.username_id,username=self.Username,password_id=self.password_id,password=self.Password,submit_id=self.submit_id)
        self.random_sleep()
        # 窗口最大化
        ...
        # 潽蓝基金
        for index,fundCode in enumerate(['999901 Plan 01 Standard','999902 Plan 02 Standard','999903 Plan 03 Standard','999904 Plan 04 Standard','999905 Plan 05 Standard','999906 Plan 06 Standard','999907 Plan 07 Standard','999908 Plan 08 Standard','999909 Plan 09 Standard','999910 Plan 10 Standard']):
            select = Select(driver.find_element_by_name('ListCustomInstrument'))
            fund_code = re.sub("\D", "", fundCode)
            fund_code = fund_code[:6]  # 有些fund需要清洗

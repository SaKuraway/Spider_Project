...
def bonusget(input_x):
    ...
    if x == 'quarterly':
        return 8
    if x == 'half-yearly' or 'semi-annual':
        return 4
    if x == 'annually' or 'annual':
        return 2
    if x == 'initial':
        return '1'
    if x == 'accumulator':
...
for policy_no in policy_no_list[1:]:
    ...
    try:
        ...
        premium_contribution_list = premium_response.xpath("//div[@class='full']/table//tr/td[3]//text()")
        invested_premium_list = premium_response.xpath("//div[@class='full']/table//tr/td[4]//text()")
        ...
        if tb_frequency and prem:
            ...
            if int(float(prem)) != int(float(before_premium)):
                ...
                try_except(cur.execute("insert into tb_account_supply(policy_no,name,platform,product,event_type,event_date,before_change,after_change) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",
                    (full_policy_no, tb_name, tb_platform, tb_product, 'premium_changed', Spider_Date, before_premium,prem)))
            ...
            if str(tb_frequency) != str(before_frequency):
                ...
                try_except(cur.execute("insert into tb_account_supply(policy_no,name,platform,product,event_type,event_date,before_change,after_change) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",
                    (full_policy_no, tb_name, tb_platform, tb_product, 'frequency_changed', Spider_Date,

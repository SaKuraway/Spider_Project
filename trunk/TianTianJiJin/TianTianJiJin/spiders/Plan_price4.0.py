...
for i in range(0,len(Switch_dates)-1):
    ...
    if i ==5:
        print(df_agg)
        print(df_agg.iloc[-1].count())
        print(len(df_agg.columns))
        print(i == len(Switch_dates)-1)
        print(len(Switch_dates)-1)
    while df_agg.iloc[-1].count()!=len(df_agg.columns) and i == len(Switch_dates)-2:
...
print(df_price_agg.loc[datetime.date(2017,9,27):datetime.date(2017,10,30)])
# upload to mysql.

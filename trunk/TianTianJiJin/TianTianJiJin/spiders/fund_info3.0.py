...
class TiantianJijinSpider(object):
    ...
    def start_url(self):
        time.sleep(3)
        start_url = 'http://fund.eastmoney.com/data/rankhandler.aspx?op=ph&dt=kf&ft=all&rs=&gs=0&sc=zzf&st=desc&qdii=&tabSubtype=,,,,,&pi=1&pn=10000&dx=0&v=0.9361817037458058'
        ...
        # 添加基金排名
        for index,fund_list in enumerate(self.funds_list):
            rank = index + 1
            fund_code = fund_list[0]
            count = self.cur.execute("update fund_information set rank='"+str(rank)+"' where fund_code='"+str(fund_code)+"';")
            print(count,"update fund_information set rank='"+str(rank)+"' where fund_code='"+str(fund_code)+"';")

        self.mysql_client.commit()
        print('rank_update committed.')
        self.cur.close()

        # fund_information_spider：
        for i in range(len(self.funds_list)):
            ...
            try:
                ...
                print('Fund_code,rank:', each[0],item['rank'])
    ...
    def parse_fundinfo(self, response, item):
        ...
        try:
            ...
            item['Fund_full_name'] = ''.join(table_item[0].xpath('.//text()')) if table_item[0] is not None else ''
        ...
        # 同类排名页面
        similar_ranking_url = 'http://fund.eastmoney.com/f10/FundArchivesDatas.aspx?type=jdzf&code={0}&rt=0.34388133943600296'.format(item['Fund_code'])
        response = etree.HTML(requests.get(url=similar_ranking_url, headers=headers).text)
        # print('~~~',requests.get(url=similar_ranking_url, headers=headers).text)
        self.similar_ranking(response,item)

    def similar_ranking(self,response,item):
        time.sleep(2)
        # fund_info_url = 'http://fund.eastmoney.com/f10/jbgk_{0}.html'.format(item['Fund_code'])
        # response = etree.HTML(requests.get(url=fund_info_url,headers=headers).text)
        print('同类排名页面')
        try:
            table_item = response.xpath("//li[@class='tlpm'][last()]")
            item['This_Year_Ranking'] = ''.join(table_item[0].xpath('.//text()')) if table_item[0] is not None else ''
            item['Nearly_1weeks_Ranking'] = ''.join(table_item[1].xpath('.//text()')) if table_item[0] is not None else ''
            item['Nearly_1months_Ranking'] = ''.join(table_item[2].xpath('.//text()')) if table_item[0] is not None else ''
            item['Nearly_3months_Ranking'] = ''.join(table_item[3].xpath('.//text()')) if table_item[0] is not None else ''
            item['Nearly_6months_Ranking'] = ''.join(table_item[4].xpath('.//text()')) if table_item[0] is not None else ''
            item['Nearly_1years_Ranking'] = ''.join(table_item[5].xpath('.//text()')) if table_item[0] is not None else ''
            item['Nearly_2years_Ranking'] = ''.join(table_item[6].xpath('.//text()')) if table_item[0] is not None else ''
            item['Nearly_3years_Ranking'] = ''.join(table_item[7].xpath('.//text()')) if table_item[0] is not None else ''
            item['Nearly_5years_Ranking'] = ''.join(table_item[8].xpath('.//text()')) if table_item[0] is not None else ''
            item['Since_Established_Ranking'] = ''.join(table_item[9].xpath('.//text()')) if table_item[0] is not None else ''
    ...
    def process_item_mysql(self, item):
        ...
        try:
            ...
            self.try_except(cur.execute("insert into Fund_information(Fund_full_name,Fund_name,Fund_code,rank,Fund_Type,Risk_Level,Inception_Date,Launch_Date,Min_Purchase,Max_Initial_Charge,Management_Fee,Max_Redemption_charge,Benchmark,Spider_Date,Unit_Value,Total_Value,Daily_Growth_Rate,Nearly_1weeks_Rate,Nearly_1months_Rate,Nearly_3months_Rate,Nearly_6months_Rate,Nearly_1years_Rate,Nearly_2years_Rate,Nearly_3years_Rate,This_Year_Rate,Since_Established_Rate,Procedures_Fee,Assets_Scale,Share_Scale,Fund_Administrator,Fund_Custodian,Custodian_Rate,This_Year_Ranking,Nearly_1weeks_Ranking,Nearly_1months_Ranking,Nearly_3months_Ranking,Nearly_6months_Ranking,Nearly_1years_Ranking,Nearly_2years_Ranking,Nearly_3years_Ranking,Nearly_5years_Ranking,Since_Established_Ranking) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                (item['Fund_full_name'],item['Fund_name'], item['Fund_code'],item['rank'],item['Fund_Type'],item['Risk_Level'], item['Inception_Date'], item['Launch_Date'], item['Min_Purchase'],
                ...
                item['Fund_Administrator'], item['Fund_Custodian'], item['Custodian_Rate'], item['This_Year_Ranking'],item['Nearly_1weeks_Ranking'],item['Nearly_1months_Ranking'],item['Nearly_3months_Ranking'],item['Nearly_6months_Ranking'],item['Nearly_1years_Ranking'],item['Nearly_2years_Ranking'],item['Nearly_3years_Ranking'],item['Nearly_5years_Ranking'],item['Since_Established_Ranking'])))

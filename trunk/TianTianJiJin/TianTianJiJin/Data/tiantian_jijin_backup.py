...
class TiantianJijinSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for list in py_json:
            ...
            # item['Nearly_1weeks_Rate'] = ''
            # item['Nearly_1months_Rate'] =''
            # item['Nearly_3months_Rate'] =''
            # item['Nearly_6months_Rate'] = ''
            # item['Nearly_1years_Rate'] = ''
            # item['Nearly_2years_Rate'] = ''
            # item['Nearly_3years_Rate'] = ''
            # item['This_Year_Rate'] =''
            # item['Since_Established_Rate'] =''
            # item['Procedures_Fee'] = ''
    ...
    def parse_fundinfo(self, response):
        ...
        try:
            ...
            item['Assets_Scale'] = ''.join(table_item[6].xpath('.//text()').extract()) if table_item[6] else ''  # 资产管理规模
            item['Share_Scale'] = ''.join(table_item[7].xpath('.//text()').extract()) if table_item[7] else ''  # 份额规模
            item['Fund_Administrator'] = ''.join(table_item[8].xpath('.//text()').extract()) if table_item[8] else ''  # 基金管理人
            item['Fund_Custodian'] = ''.join(table_item[9].xpath('.//text()').extract()) if table_item[9] else ''  # 基金托管人
            item['Management_Fee'] = ''.join(table_item[12].xpath('.//text()').extract()) if table_item[12] else ''  # 管理费率
            item['Custodian_Rate'] = ''.join(table_item[13].xpath('.//text()').extract()) if table_item[13] else ''  # 托管费率
            ...
            with open('Data/' + '错误日志' + '.log', 'a', encoding='utf-8') as f:
                ...
                f.write(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + '：' + '正在写入%s的错误信息：fundCode=%s..'%(str(sys._getframe().f_code.co_name),item['Fund_code']) + str(e))
            f.close()
    ...
    def parse_fund_manger_change(self, response):
        ...
        try:
            ...
            item['Fund_managers'] = [''.join(i.xpath('.//text()').extract()) if i.xpath('.//text()').extract() else ''
    ...
    def parse_history_NAV(self, response):
        ...
        item['Purchase_status'] = [i.xpath('.//text()').extract_first() if i.xpath('.//text()').extract_first() else ''
                                   for i in response.xpath("//table[@class='w782 comm lsjz']/tbody/tr/td[5]")] if response.xpath("//table[@class='w782 comm lsjz']/tbody/tr/td[7]") else [i.xpath('.//text()').extract_first() if i.xpath('.//text()').extract_first() else ''
        ...
        item['Redeem_status'] = [i.xpath('.//text()').extract_first() if i.xpath('.//text()').extract_first() else ''
                                 for i in response.xpath("//table[@class='w782 comm lsjz']/tbody/tr/td[6]")] if response.xpath("//table[@class='w782 comm lsjz']/tbody/tr/td[7]") else [i.xpath('.//text()').extract_first() if i.xpath('.//text()').extract_first() else ''
                                 for i in response.xpath("//table[@class='w782 comm lsjz']/tbody/tr/td[5]")]  # 赎回状态
        item['Dividends_distribution'] = [
            i.xpath('.//text()').extract_first() if i.xpath('.//text()').extract_first() else '' for i in
            response.xpath("//table[@class='w782 comm lsjz']/tbody/tr/td[7]")] if response.xpath("//table[@class='w782 comm lsjz']/tbody/tr/td[7]") else [i.xpath('.//text()').extract_first() if i.xpath('.//text()').extract_first() else ''

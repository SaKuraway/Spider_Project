...
class AutoOperation(object):
        ...
        for index, item in enumerate(ITA):
            ...
            try:
                ...
                try:
                    # 点击完成
                    self.random_sleep()
                    driver.find_element_by_xpath("//div[@id='Step5panel']/div[@id='stepsTransferRebalance_5']/div[@class='panel-body form']/div[@class='form-actions']/a").click()
                    # self.wait_until(driver=driver,platform='ITA',
                    #                 selector_name="//div[@id='Step5panel']/div[@id='stepsTransferRebalance_5']/div[@class='panel-body form']/div[@class='form-actions']/a")
                    # 点击两个缩进箭头
                    driver.find_element_by_xpath("//div[@class='portlet box grey'][1]/div[@class='portlet-title']/div[@class='tools']/a").click()
                    ...
                    self.random_sleep()
                    driver.find_element_by_xpath("//div[@id='GraphBenefitsPorlet']/div[@class='portlet-title']/div[@class='tools']/a").click()
                    ...
                    if self.error == 0:
                        print('购买ITA保单后报错了,可能截图有问题',self.policyNumber)
                    else:
                        print('error!!')
                self.random_sleep()
                # 调仓完毕，截图保存。
    ...
    def zurich_operation(self, zurich):
        ...
        for index,item in enumerate(str(PIN)):
            ...
            self.random_sleep()
        driver.find_element_by_id("ctl00_ContentPlaceHolder1_ucPinValidator_btnRandomPinContinue").click()
    ...
    def check_workoff(self, off_time=2130):
        """

...
class TiantianjijinMysqlPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            for Fund_size_change_Date,Period_Purchase,Period_Redeem,Ending_shares,Ending_net_asset,Net_asset_change in zip(item['Fund_size_change_Date'],item['Period_Purchase'],item['Period_Redeem'],item['Ending_shares'],item['Ending_net_asset'],item['Net_asset_change']):
                if not cur.execute("select * from Fund_size_change where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Fund_size_change_Date=" + "'" +Fund_size_change_Date + "'" + ";"):
                    self.try_except(cur.execute("insert into Fund_size_change(Fund_code,Fund_size_change_Date,Period_Purchase,Period_Redeem,Ending_shares,Ending_net_asset,Net_asset_change) VALUES(%s,%s,%s,%s,%s,%s,%s)",(item['Fund_code'],Fund_size_change_Date,Period_Purchase,Period_Redeem,Ending_shares,Ending_net_asset,Net_asset_change)))
                # else:
            ...
            for Fund_holder_Date,Institution,Individual,Internal,Total_shares in zip(item['Fund_holder_Date'],item['Institution'],item['Individual'],item['Internal'],item['Total_shares']):
                if not cur.execute("select * from Fund_holder where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Fund_holder_Date=" + "'" + Fund_holder_Date + "'" + ";"):
                    self.try_except(cur.execute("insert into Fund_holder(Fund_code,Fund_holder_Date,Institution,Individual,Internal,Total_shares) VALUES(%s,%s,%s,%s,%s,%s)",(item['Fund_code'],Fund_holder_Date,Institution,Individual,Internal,Total_shares)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            for Fund_holding_season,Fund_holding_id,Stock_code,Stock_name,Single_stock_percent,Stock_holding_quantity,Stock_holding_value in zip(item['Fund_holding_season'],item['Fund_holding_id'],item['Stock_code'],item['Stock_name'],item['Single_stock_percent'],item['Stock_holding_quantity'],item['Stock_holding_value']):
                if not cur.execute("select * from Fund_holding where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Fund_holding_season=" + "'" + Fund_holding_season + "'"+ " and Fund_holding_id=" + "'" + Fund_holding_id + "'" + ";"):
                    self.try_except(cur.execute("insert into Fund_holding(Fund_code,Fund_holding_season,Fund_holding_id,Stock_code,Stock_name,Single_stock_percent,Stock_holding_quantity,Stock_holding_value) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",(item['Fund_code'],Fund_holding_season,Fund_holding_id,Stock_code,Stock_name,Single_stock_percent,Stock_holding_quantity,Stock_holding_value)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            for Transaction_details_season,Transaction_details_id,Buying_stock_code,Buying_stock_name,Accumulated_buy_value,Accumulated_buy_percent_of_NAV in zip(item['Transaction_details_season'],item['Transaction_details_id'],item['Buying_stock_code'],item['Buying_stock_name'],item['Accumulated_buy_value'],item['Accumulated_buy_percent_of_NAV']):
                if not cur.execute("select * from Transaction_details where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Transaction_details_season=" + "'" + Transaction_details_season + "'"+ " and Transaction_details_id=" + "'" + Transaction_details_id + "'" + ";"):
                    self.try_except(cur.execute("insert into Transaction_details(Fund_code,Transaction_details_season,Transaction_details_id,Buying_stock_code,Buying_stock_name,Accumulated_buy_value,Accumulated_buy_percent_of_NAV) VALUES(%s,%s,%s,%s,%s,%s,%s)",(item['Fund_code'],Transaction_details_season,Transaction_details_id,Buying_stock_code,Buying_stock_name,Accumulated_buy_value,Accumulated_buy_percent_of_NAV)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            for Setors_season,Setors_id,Setors,Setors_percent,Setors_value in zip(item['Setors_season'],item['Setors_id'],item['Setors'],item['Setors_percent'],item['Setors_value']):
                if not cur.execute("select * from Setors_configuration where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Setors_season=" + "'" + Setors_season + "'"+ " and Setors_id=" + "'" + Setors_id + "'" + ";"):
                    self.try_except(cur.execute("insert into Setors_configuration(Fund_code,Setors_season,Setors_id,Setors,Setors_percent,Setors_value) VALUES(%s,%s,%s,%s,%s,%s)",(item['Fund_code'],Setors_season,Setors_id,Setors,Setors_percent,Setors_value)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            for Comparation_date,CSRC_setors_code,Setors_name,Fund_setor_weight,Similarfund_setor_weight,Fund_setor_different in zip(item['Comparation_date'],item['CSRC_setors_code'],item['Setors_name'],item['Fund_setor_weight'],item['Similarfund_setor_weight'],item['Fund_setor_different']):
                if not cur.execute("select * from Setors_comparation where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Comparation_date=" + "'" + Comparation_date + "'"+ " and CSRC_setors_code=" + "'" + CSRC_setors_code + "'" + ";"):
                    self.try_except(cur.execute("insert into Setors_comparation(Fund_code,Comparation_date,CSRC_setors_code,Setors_name,Fund_setor_weight,Similarfund_setor_weight,Fund_setor_different) VALUES(%s,%s,%s,%s,%s,%s,%s)",(item['Fund_code'],Comparation_date,CSRC_setors_code,Setors_name,Fund_setor_weight,Similarfund_setor_weight,Fund_setor_different)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            for Style_season,Style_box in zip(item['Style_season'],item['Style_box']):
                if not cur.execute("select * from StyleBox where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Style_season=" + "'" + Style_season + "'"+ " and Style_box=" + "'" + Style_box + "'" + ";"):
                    self.try_except(cur.execute("insert into StyleBox(Fund_code,Style_season,Style_box) VALUES(%s,%s,%s)",(item['Fund_code'],Style_season,Style_box)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            if not cur.execute("select * from Tracking where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Tracking_index=" + "'" + item['Tracking_index'] + "'"+ " and Tracking_error=" + "'" + item['Tracking_error'] + "'" + ";"):
                self.try_except(cur.execute("insert into Tracking(Fund_code,Tracking_index,Tracking_error,Similar_average_tracking_error) VALUES(%s,%s,%s,%s)",(item['Fund_code'],item['Tracking_index'],item['Tracking_error'],item['Similar_average_tracking_error'])))
            # else:
            #     print('Same to Mysql ,skipping it!')
            for Start_date,Ending_date,Fund_managers,Appointment_time,Appointment_return in zip(item['Start_date'],item['Ending_date'],item['Fund_managers'],item['Appointment_time'],item['Appointment_return']):
                if cur.execute("select * from Current_fund_manager_change where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Start_date=" + "'" + Start_date + "'"+ ";"):
                    self.try_except(cur.execute("delete from Current_fund_manager_change where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Start_date=" + "'" + Start_date + "'"+ ";"))
                self.try_except(cur.execute("insert into Current_fund_manager_change(Fund_code,Start_date,Ending_date,Fund_managers,Appointment_time,Appointment_return) VALUES(%s,%s,%s,%s,%s,%s)",(item['Fund_code'],Start_date,Ending_date,Fund_managers,Appointment_time,Appointment_return)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            for Held_fund_code,Held_fund_name,Held_fund_type,Held_fund_start_time,Held_fund_end_time,Held_fund_appointment_time,Held_fund_repayment,Held_fund_similar_average,Held_fund_similar_ranking in zip(item['Held_fund_code'],item['Held_fund_name'],item['Held_fund_type'],item['Held_fund_start_time'],item['Held_fund_end_time'],item['Held_fund_appointment_time'],item['Held_fund_repayment'],item['Held_fund_similar_average'],item['Held_fund_similar_ranking']):
                if not cur.execute("select * from All_fund_managers where Current_fund_manager=" + "'" + item['Current_fund_manager'] + "'" + " and Held_fund_code=" + "'" + Held_fund_code + "'"+ " and Held_fund_start_time=" + "'" + Held_fund_start_time + "'"+ " and Held_fund_repayment=" + "'" + Held_fund_repayment + "'" + ";"):
                    self.try_except(cur.execute("insert into All_fund_managers(Current_fund_manager,Held_fund_code,Held_fund_name,Held_fund_type,Held_fund_start_time,Held_fund_end_time,Held_fund_appointment_time,Held_fund_repayment,Held_fund_similar_average,Held_fund_similar_ranking) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(item['Current_fund_manager'],Held_fund_code,Held_fund_name,Held_fund_type,Held_fund_start_time,Held_fund_end_time,Held_fund_appointment_time,Held_fund_repayment,Held_fund_similar_average,Held_fund_similar_ranking)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            ...
            for Fund_allocations_Date,Fund_allocations_Stock_percent,Fund_allocations_Bond_percent,Cash_percent,Net_asset in zip(item['Fund_allocations_Date'],item['Fund_allocations_Stock_percent'],item['Fund_allocations_Bond_percent'],item['Cash_percent'],item['Net_asset']):
                if not cur.execute("select * from Fund_allocations where Fund_code=" + "'" + item['Fund_code'] + "'" + " and Fund_allocations_Date=" + "'" + Fund_allocations_Date + "'" + ";"):
                    self.try_except(cur.execute("insert into Fund_allocations(Fund_code,Fund_allocations_Date,Fund_allocations_Stock_percent,Fund_allocations_Bond_percent,Cash_percent,Net_asset) VALUES(%s,%s,%s,%s,%s,%s)",(item['Fund_code'],Fund_allocations_Date,Fund_allocations_Stock_percent,Fund_allocations_Bond_percent,Cash_percent,Net_asset)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            for History_NAV_Date,NAV,ANAV,Day_change,Purchase_status,Redeem_status,Dividends_distribution in zip(item['History_NAV_Date'],item['NAV'],item['ANAV'],item['Day_change'],item['Purchase_status'],item['Redeem_status'],item['Dividends_distribution']):
                if not cur.execute("select * from History_NAV where Fund_code=" + "'" + item['Fund_code'] + "'" + " and History_NAV_Date=" + "'" + History_NAV_Date + "'" + ";"):
                    self.try_except(cur.execute("insert into History_NAV(Fund_code,History_NAV_Date,NAV,ANAV,Day_change,Purchase_status,Redeem_status,Dividends_distribution) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",(item['Fund_code'],History_NAV_Date,NAV,ANAV,Day_change,Purchase_status,Redeem_status,Dividends_distribution)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            try:
                for ACWorth_date,this_fund_rate,similar_fund_rate,hs300_rate in zip(item['ACWorth_date'],item['this_fund_rate'],item['similar_fund_rate'],item['hs300_rate']):
                    if not cur.execute("select * from ACWorth where Fund_code=" + "'" + item['Fund_code'] + "'" + " and ACWorth_date=" + "'" + ACWorth_date + "'" + ";"):
                        self.try_except(cur.execute("insert into ACWorth(Fund_code,ACWorth_date,this_fund_rate,similar_fund_rate,hs300_rate) VALUES(%s,%s,%s,%s,%s)",(item['Fund_code'],ACWorth_date,this_fund_rate,similar_fund_rate,hs300_rate)))
                # else:
                #     print('Same to Mysql ,skipping it!')
            except:
                pass
    ...
    def close_spider(self, spider):
        tableNameList = ['Fund_information','Fund_size_change','Fund_holder','Fund_holding','Transaction_details','Setors_configuration','Setors_comparation','StyleBox','Tracking','Current_fund_manager_change','All_fund_managers','Main_financial_index','Assets_balance_sheet','Income_statements','Income_analysis','Expenses_analysis','Fund_allocations','History_NAV','ACWorth']
        print('正在排序数据表中的id列....')
        cur = self.mysql_client.cursor()

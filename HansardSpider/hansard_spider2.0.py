...
for policy_no in policy_no_list[1:]:
    ...
    try:
        ...
        tb_frequency = frequency_to_num(''.join(premium_response.xpath("//div[@class='results']/div[@class='full list_2_col'][1]/ul/li[last()-3]/text()")).strip().replace(' ',''))
        ...
        if tb_frequency:
            ...
            if prem != before_premium:
                ...
                print("insert into tb_account_supply(policy_no,name,platform,product,event_type,event_date,before_change,after_change) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",
                    (full_policy_no,tb_name,tb_platform, tb_product,'premium_changed',Spider_Date,float(before_premium),prem))
            ...
            before_frequency = frequency_to_num(before_frequency)
            if tb_frequency != before_frequency:
                print(full_policy_no,'frequency changed!')
                try_except(cur.execute("insert into tb_account_supply(policy_no,name,platform,product,event_type,event_date,before_change,after_change) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)",
                    (full_policy_no,tb_name,tb_platform, tb_product,'frequency_changed',Spider_Date,before_frequency,frequency)))
                cur.execute("update tb_client set payment_frequency = '"+str(tb_frequency)+"' where policy_no='" + full_policy_no + "';")
        ...
        if not cur.execute("select * from tb_premium where policy_no='" + full_policy_no + "' and Spider_Date='" + Spider_Date + "';"):
            cur.execute("select date from tb_premium where policy_no='" + full_policy_no + "';")
            ...
            for premium_date,premium_status,premium_currency,premium_contribution in zip(premium_date_list,premium_status_list,premium_currency_list,premium_contribution_list):
                ...
                if i == 0:
                    ...
                    print('bouns:',bouns)
                if date_to_num(premium_date) not in existed_date:
                    ...
                    try_except(cur.execute("insert into tb_premium(policy_no,Spider_Date,date,type,status,currency,value,bonus,frequency) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                        (full_policy_no,Spider_Date,date_to_num(premium_date),1 if float(premium_contribution.replace(',',''))>=0 else 2,2 if premium_status=='Payment Processed' else 1,premium_currency,premium_contribution.replace(',',''),bouns,frequency)))

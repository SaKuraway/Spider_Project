        ...
        for csv_date in range(1,32):
            ...
            try:
                ...
                if self.sheet_name == 1:
                    # 需要删掉csv里面的policy number的第一个空格
                    csv_data = pd.read_csv('./Excel/policy/201801/SLA/Fund/SLA fund for cbc 201801'+str(csv_date)+'.csv')
                else:
                    csv_data = pd.read_csv('./Excel/policy/201801/SLA/Policy/SLA policy for cbc 201801'+str(csv_date)+'.csv')
                result = []

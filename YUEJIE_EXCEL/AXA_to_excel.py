    ...
    def read_from_csv(self):
        ...
        for csv_date in range(1, 32):
            try:
                ...
                csv_data = pd.read_csv('./Excel/policy/201801/AXA/axa policy for cbc 201801' + str(csv_date) + '.csv')
                result = []
                for index in range(csv_data.iloc[:, 0].size):
                    ...
                    # 获取查询的结果
                    result.append((policy, value))
                    # result = [('562499G','2017-11-30',13333.32),('562499G','2017-12-28',3333.33)]
                # 打印查询的结果
                for policy, value in result:
                    # print(type(policy),type(date),type(value))
                    print(policy, value)
                    if '2018-01-' + str(csv_date) in weekday.keys():
                        self.read_from_excel(policy, str('2018-01-' + str(csv_date)), value)
            except:
...
if __name__ == '__main__':
    ...
    yuejie_data_baofei = Csv_To_Excel(1)
    yuejie_data_baofei.read_from_csv()
